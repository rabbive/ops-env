...
- Root `inference.py` emits machine-parseable `[START]` / `[STEP]` / `[END]` JSON lines on stdout; human-readable progress goes to stderr by default (`INFERENCE_VERBOSE=0` silences it). `.env.example` documents `API_BASE_URL`, `MODEL_NAME`, `HF_TOKEN`, and optional overrides such as `ENV_BASE_URL`.
